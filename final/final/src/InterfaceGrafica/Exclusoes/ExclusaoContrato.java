...
public class ExclusaoContrato extends JFrame {
    JComboBox<Contrato> exclusao = new JComboBox<>();
    JButton btn1 = new JButton();
    private static Contrato item_selecionado;

    public ExclusaoContrato(){
        setTitle("Exclusão de Contratos");
        setVisible(true);
        setSize(800, 500);
        setResizable(false);
        ...
        JLabel titulo = new JLabel("Exclusão de Contratos");
    ...
    public void btn1(ActionEvent ActionEvent){
        item_selecionado = (Contrato) exclusao.getSelectedItem();

        if(item_selecionado!=null) {
            App.Contratos.remove(item_selecionado);
